...
describe('oidc-token-client', () => {
  describe('exchangeAuthorizationCode', () => {
    it('POSTs the urlencoded auth-code body to /api/v1/identities/token and returns the parsed response', async () => {
      ...
      const result = await exchangeAuthorizationCode(
        {
          apiBaseUrl: 'http://localhost:15201',
      ...
      expect(url).toBe('http://localhost:15201/api/v1/identities/token');
  ...
  describe('exchangeRefreshToken', () => {
    it('POSTs the urlencoded refresh-token body to /api/v1/identities/token', async () => {
      const fetchImpl = vi.fn().mockResolvedValue(
